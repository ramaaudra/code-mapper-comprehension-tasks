...
export interface DependencyNodeData extends Record<string, unknown> {
  ...
  subtitle?: string;
  badges?: Array<{ label: string; tone: 'info' | 'warning' | 'danger' | 'success' }>;
  isHovered?: boolean;
}
...
function layoutNodes(
  ...
  const normalizedHover = hoveredFile ? normalizePath(hoveredFile).toLowerCase() : null;
  const normalizedHoverBasename = normalizedHover ? getBasename(normalizedHover).toLowerCase() : null;
  const normalizedFocus = focusNodeId ? normalizePath(focusNodeId).toLowerCase() : null;
  ...
  return nodes.map(node => {
    ...
    const isHovered = Boolean(
      normalizedHover &&
      (normalizedHover === normalizedNodePath || normalizedHoverBasename === normalizedNodeBasename)
    );
    const isFocused = normalizedFocus ? normalizedFocus === normalizedNodePath : false;
...
function ZoomIndicator() {
  ...
  const label = percentage < 75 ? 'Wide view' : percentage > 175 ? 'Close view' : 'Normal';
...
function DependencyNodeComponent(props: NodeProps<DependencyFlowNode>) {
  ...
  const direction = (data.direction ?? 'placeholder') as DependencyNodeData['direction'];

  const backgroundTone: Record<DependencyNodeData['direction'], string> = {
    selected: 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-400 dark:border-emerald-500',
    incoming: 'bg-blue-50 dark:bg-blue-900/20 border-blue-300 dark:border-blue-500/60',
    outgoing: 'bg-amber-50 dark:bg-amber-900/20 border-amber-300 dark:border-amber-500/60',
    placeholder: 'bg-slate-50 dark:bg-slate-800/30 border-slate-300 dark:border-slate-700',
  };
  ...
  return (
    <div
      className={clsx(
        ...
        data.isHovered && 'ring-2 ring-emerald-500/70 ring-offset-2 ring-offset-slate-900/0',
      )}
      tabIndex={0}
      role="button"
    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <div className={clsx(
          ...
          {direction === 'selected' ? 'Focus' : direction === 'incoming' ? 'Importer' : direction === 'outgoing' ? 'Dependency' : 'Info'}
        </div>
      </div>
          ...
          {data.badges.map((badge) => (
            <span
              ...
              className={clsx(
                ...
                badge.tone === 'danger' && 'bg-red-500/10 text-red-600 dark:text-red-300',
                badge.tone === 'warning' && 'bg-amber-500/10 text-amber-600 dark:text-amber-300',
                badge.tone === 'success' && 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-300',
                badge.tone === 'info' && 'bg-blue-500/10 text-blue-600 dark:text-blue-300',
              )}
            >
              {badge.label}
            </span>
      ...
      <Handle type="target" position={Position.Left} style={{ opacity: 0, width: 12, height: 12, pointerEvents: 'none' }} />
      <Handle type="target" position={Position.Top} style={{ opacity: 0, width: 12, height: 12, pointerEvents: 'none' }} />
      <Handle type="source" position={Position.Right} style={{ opacity: 0, width: 12, height: 12, pointerEvents: 'none' }} />
      <Handle type="source" position={Position.Bottom} style={{ opacity: 0, width: 12, height: 12, pointerEvents: 'none' }} />
    </div>
  );
}
...
function DependencyGraphInner({
  ...
  const [flowNodes, setNodes, onNodesChange] = useNodesState<DependencyFlowNode>(layoutedNodes);
  const [flowEdges, setEdges, onEdgesChange] = useEdgesState<DependencyFlowEdge>(edges);
  ...
  const handleNodeClick = useCallback((_: any, node: DependencyFlowNode) => {
  ...
  return (
    <div className="relative w-full h-full graph-container">
      ...
      <ZoomControls
        ...
        onReset={() => reactFlow.setViewport({ x: 0, y: 0, zoom: 1 }, { duration: 200 })}
  ...
  if (!props.nodes.length) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center gap-3 text-slate-500 dark:text-slate-400">
        <h2 className="text-lg font-semibold text-slate-700 dark:text-slate-100">Tidak ada dependensi untuk file ini</h2>
        <p className="text-sm max-w-md">
          Pilih file lain dari tree untuk melihat hubungan import/export yang lebih kompleks.
        </p>
      </div>
    );
  }
